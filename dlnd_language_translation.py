...
def text_to_ids(source_text, target_text, source_vocab_to_int, target_vocab_to_int):
    ...
    for sentence_ids in target_id_text:
...
def model_inputs():
    ...
    source_length = tf.placeholder(tf.int32,shape=(None,),name='source_sequence_length')
    
    return inputs, targets, learning_rate, keep_probability, target_length, max_length, source_length
...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob, 
    ...
    def make_cell(rnn_size):
        enc_cell = tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.LSTMCell(rnn_size,initializer=tf.random_uniform_initializer(-0.1,0.1,seed=2)))
        return enc_cell
...
def sentence_to_seq(sentence, vocab_to_int):
    ...
    for word in sentence.lower().split():
        id = vocab_to_int.get(word,vocab_to_int['<UNK>'])
        output.append(id)
